...
public:
    ...
    bool onMessage(const StockDirectoryMessage& message) { ++_messages; _market.AddOrderBook(message.StockLocate); return true; }
    ...
    bool onMessage(const AddOrderMessage& message) { ++_messages; _market.AddOrder(message.OrderReferenceNumber, message.StockLocate, (message.BuySellIndicator == 'B') ? (int32_t)message.Price : -((int32_t)message.Price), message.Shares); return true; }
    bool onMessage(const AddOrderMPIDMessage& message) { ++_messages; _market.AddOrder(message.OrderReferenceNumber, message.StockLocate, (message.BuySellIndicator == 'B') ? (int32_t)message.Price : -((int32_t)message.Price), message.Shares); return true; }
    bool onMessage(const OrderExecutedMessage& message) { ++_messages; _market.ExecuteOrder(message.OrderReferenceNumber, message.ExecutedShares); return true; }
    bool onMessage(const OrderExecutedWithPriceMessage& message) { ++_messages; _market.ExecuteOrder(message.OrderReferenceNumber, message.ExecutionPrice, message.ExecutedShares); return true; }
